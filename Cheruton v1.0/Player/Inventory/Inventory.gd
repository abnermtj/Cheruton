...
func revert_item_state():
	...
	# Initially Fixed
		...
		item_state = "HOVER"
		get_node("Bg/Contents/EquippedCoins/Button").hide()
		mouse_node = null
...
func delete_item():
	...
	if (DataResource.dict_inventory[active_tab.name]["Item" + element_index].item_qty != 0):
		...
		# Dequips active item
		var main = get_node("Bg/Contents/Items/" + active_tab.name)
		if(active_tab.name == "Weapons" || active_tab.name == "Apparel"):
			if(DataResource.temp_dict_player[active_tab.name + "_item"] == mouse_node.name):
				...
				get_node("Bg/Contents/EquippedCoins/" + active_tab.name).hide()
...
func item_status(selected_node, status):
	var type = get_node("Bg/Contents/EquippedCoins/" + active_tab.name)
	match status:
		"EQUIP":
